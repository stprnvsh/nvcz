...
#include <nvcomp/gdeflate.hpp>               // nvcomp::GdeflateManager
...
struct NvcompGDeflate : Codec {
  ...
  nvcomp::GdeflateManager& get_manager(cudaStream_t s) {
    auto it = mgr_by_stream.find(s);
    if (it != mgr_by_stream.end()) return static_cast<nvcomp::GdeflateManager&>(*it->second);
  ...
  // New contract:
  // - Writes the exact compressed size into d_comp_size (device-side).
  // - Copies the *max-bound* number of bytes from device output into host 'dst'
  //   so the caller can later write exactly the first 'comp_len' bytes (read
  //   from a pinned host size_t after waiting on the stream event).
  // - No host-side size queries and no stream syncs here.
                            ...
                            size_t* d_comp_size) override
  {
    ...
    void* d_in  = nullptr;
    void* d_out = nullptr;
    cuda_ck(cudaMallocAsync(&d_in,  n, s), "gdef d_in");
    cuda_ck(cudaMallocAsync(&d_out, cfg.max_compressed_buffer_size, s), "gdef d_out");

    // H2D input
    cuda_ck(cudaMemcpyAsync(d_in, src, n, cudaMemcpyHostToDevice, s), "gdef H2D");

    // Compress (nvCOMP will write the exact size to d_comp_size on device)
    ...
    // Copy the max-bound payload back to host. The writer will only emit the
    // first 'comp_len' bytes after it reads the pinned size_t (post-event).
    cuda_ck(cudaMemcpyAsync(
        dst,
        d_out,
        cfg.max_compressed_buffer_size,
        cudaMemcpyDeviceToHost,
        s), "gdef D2H (max-bound)");

    // Return buffers to the async allocator when the stream reaches this point.
    cuda_ck(cudaFreeAsync(d_in,  s), "gdef free d_in");
    cuda_ck(cudaFreeAsync(d_out, s), "gdef free d_out");
  }

  // Decompress single buffer on the provided CUDA stream (no syncs here).
                              ...
                              cudaStream_t s) override
  {
    ...
    void* d_in = nullptr;
    cuda_ck(cudaMallocAsync(&d_in, comp_n, s), "gdef d_in (comp)");
    cuda_ck(cudaMemcpyAsync(d_in, comp, comp_n, cudaMemcpyHostToDevice, s), "gdef H2D comp");
    ...
    // Configure decompression (comp_size not needed for NVCOMP_NATIVE)
    ...
    void* d_out = nullptr;
    cuda_ck(cudaMallocAsync(&d_out, raw_n, s), "gdef d_out (raw)");
    ...
    // D2H the decompressed payload
    cuda_ck(cudaMemcpyAsync(dst, d_out, raw_n, cudaMemcpyDeviceToHost, s), "gdef D2H raw");

    // Free when the stream reaches these operations
    cuda_ck(cudaFreeAsync(d_in,  s), "gdef free d_in");
    cuda_ck(cudaFreeAsync(d_out, s), "gdef free d_out");
  }
