...
struct NvcompZstd : Codec {
                            ...
                            size_t* d_comp_size) override
  {
    ...
    if (!mgr_cache) {
      mgr_cache = std::make_shared<nvcomp::ZstdManager>(
          chunk_size, opts, s, checksum_policy, nvcomp::BitstreamKind::NVCOMP_NATIVE);
    }
    nvcomp::CompressionConfig cfg = static_cast<nvcomp::ZstdManager&>(*mgr_cache).configure_compression(n);
    ...
    static_cast<nvcomp::ZstdManager&>(*mgr_cache).compress(
