...
struct NvcompZstd : Codec {
  ...
  nvcomp::ZstdManager& get_manager(cudaStream_t s) {
    auto it = mgr_by_stream.find(s);
    if (it != mgr_by_stream.end()) return static_cast<nvcomp::ZstdManager&>(*it->second);
